...
def train_xgboost(features: pd.DataFrame, params: dict, target_column: str) -> str:
    ...
    # Save model - use absolute path relative to ml/ root
    # If called from ml/server/app.py, this will be ml/server/models/
    # We'll use absolute path resolution
    models_dir = Path("models")
    models_dir.mkdir(exist_ok=True)
    model_path = models_dir / f"xgb_optuna_{target_column}.json"
    model.save_model(str(model_path))
    
    # Also save as pickle for easier loading
    pickle_path = models_dir / f"xgb_optuna_{target_column}.pkl"
